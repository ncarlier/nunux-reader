...
User.login = function(user, done) {
  var autoGrantAccess = process.env.APP_AUTO_GRANT_ACCESS === 'true';
  if (autoGrantAccess) {
...
/**
 * Delete an user.
 * Also remove arcived articles.
 * @param {String}   uid  User ID.
 * @param {Function} done Callback with no params.
 */
User.remove = function(uid, done) {
  ...
  async.waterfall(
    ...
    function(err) {
      logger.error('Error will User.remove: %s', err);
      done(err);
    }
  );
};
