...
User.getTimelineSize = function(uid, done) {
  db.zcard(User.getTimelineKey(uid), function(err, reply) {
    if (err) return done(err);
 ...
 */
User.getTimeline = function(uid, start, size, done) {
  size |= '10';
  async.waterfall(
    [
      function(callback) {
        ...
        if (start) {
          db.zrank(User.getTimelineKey(uid), start, callback);
        } else {
          callback(null, null);
        }
      },
      function(rank, callback) {
        ...
        db.zrange(User.getTimelineKey(uid), rank, limit, callback);
      },
