...
User.create = function(user, done) {
  ...
  user.registrationDate = new Date().getTime().toISOString();
...
/**
 * Get all user's timeline size.
 * @param {String}   uid      User ID.
 * @param {Function} done     Callback with all user's timeline size in params.
 */
User.getAllTimelinesSize = function(uid, done) {
  var result = [];
  async.waterfall(
    [
      function(callback) {
      ...
      function(timelines, callback) {
        // Add feed timeline
        var addToResult = function(timeline, next) {
          Timeline.getSize(timeline, function(err, reply) {
            // Evict user part from timeline key
            var idx = timeline.indexOf(':', 5) + 1;
            reply.timeline = timeline.substring(idx);
            result.push(reply);
            next();
          });
        }
        async.map(timelines, addToResult, callback);
      },
      function() {
        done(null, result);
      }
    ...
    function(err) {
      console.log('Error will User.getAllTimelinesSize: %s', err);
      done(err);
    }
  );
}
 ...
 * @param {String}   uid      User ID.
 * @param {String}   timeline Timeline name.
 * @param {Object}   options  Options: next, size, order, all .
